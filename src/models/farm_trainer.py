...
class FARMTrainer:
    ...
    def _prepare_table_data(self):
        ...
        curated_files = [os.path.join(self.file_config.extracted_tables_dir, file) for file in curated["Table_filename"]]
        ...
        data = shuffle(data)
        data_train, data_dev = train_test_split(data, test_size=self.file_config.dev_split)
    ...
    def create_tokenizer(self):
        """
        Load FARM tokenizer according to language model specified in config
        object
    ...
    def create_processor(self, tokenizer):
        """
        Creates FARM processor instance. Loads the class of processor specified by
        processor name in corresponding config object. Infers the specific type
        of Processor from a config file placed in the specified directory
    ...
    def create_silo(self, processor):
        """
        Creates a FARM DataSilo instance which generates and stores PyTorch
        DataLoader objects for the train, dev and test datasets.
        Relies upon functionality in the processor to do the
        conversion of the data. Will also calculate and display some statistics.
    ...
    def create_head(self):
        """
        Creates a FARM Prediction Head which takes word embeddings from a
        language model and generates logits for a given task. Can also convert
        logits to loss and and logits to predictions.
        """
    ...
    def create_trainer(self, model, optimizer, lr_schedule, data_silo, device, n_gpu):
        """
        Creates FARM Trainer instance that handles the main model training
        procedure. This includes performing evaluation on the dev set at regular
        ...
        """
        if self.training_config.run_hyp_tuning == True:
    ...
    def _train_on_split(self, data_silo, silo_to_use, num_fold, device, n_gpu):
        """
        Trains a model for one split of cross-validation
    ...
    def run_cv(self, data_silo, xval_folds, device, n_gpu):
        """
        Trains the model and evaluates it for each fold of cross-validation.
    ...
    def run(self, trial=None, metric="acc"):
        """
        Runs model training using created FARM component. Saves a checkpoint for
        model and processor if hyperparameter tuning or cross-validation modes
        are not on. If hyperparameter tuning mode is specified in corresponding
        config object, this methods will run a trial defined by Optuna
