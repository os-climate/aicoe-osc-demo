...
class TextKPIInferenceCurator(BaseKPIInferenceCurator):
    ...
    def read_agg(self):
        """
        Read aggregated annotation csv. If doesn't exist, will create it from
        annotation folder.
        Returns:
            df (a pd dataframe)
        """
    ...
    def create_unanswerable(self, annotation_df):
        """Method responsible for creating unanswerable examples.
        ...
        for fpath in relevance_results:
            df = pd.read_csv(
                fpath, header=0, index_col=0, usecols=[0,1,2,3,4]
            )
            df_list.append(df)
    ...
    def filter_relevant_examples(self, annotation_df, relevant_df):
        """This method is responsible for filtering relevant examples that are
        mentioned in the annotation files. For each source pdf, exclude relative
        examples which their pages are mentioned in the annotation file
        Args:
