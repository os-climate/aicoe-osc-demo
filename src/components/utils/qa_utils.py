"""Util functions for QA model training."""
...
def aggregate_excels(annotation_folder, columns_to_read):
    """Read annotations from excels into one dataframe."""
    ...
    for f in excels:
        ...
        cols = df.columns
        assert all(
            [e in cols for e in columns_to_read]
...
def clean_annotation(df, save_path, kpi_category, exclude=["CEZ"]):
    """Return a clean dataframe and save it after.
    ...
    if diff > 0:
        logger.info("Drop {} examples due to incorrect kpi-data_type pair".format(diff))
